...
import { debit, credit } from '../wallet/walletService';
import { LockedRate } from '../fx/fxEngine';
import { config } from '../config/env';
import { checkSuspiciousPattern } from '../fraud/fraudDetectionService';
...
export async function validateWithdrawal(
  ...
  const userResult = await pool.query<{
    ...
    `SELECT kyc_status, daily_limit_usdc FROM users WHERE id = $1`,
    [userId]
  );
  ...
  // 2. Check for suspicious withdrawal patterns (fraud detection)
  // Requirements: 10.5
  await checkSuspiciousPattern(userId, req.amountUsdc);

  // 3. Check daily withdrawal limit (rolling 24h)
  const dailyResult = await pool.query<{ total: string }>(
  ...
  if (dailyUsed + requested > dailyLimit) {
    throw Object.assign(
      new Error(`Daily withdrawal limit of ${dailyLimit} USDC exceeded`),
      { statusCode: 422 }
    );
  }

  // 4. Fetch locked rate from Redis
  const cached = await redis.get(REDIS_KEYS.RATE_LOCK(req.lockId));
  if (!cached) {
    throw Object.assign(new Error('Rate lock expired or not found'), { statusCode: 422 });
  }

  const lockedRate: LockedRate = JSON.parse(cached);
  ...
  const walletResult = await pool.query<{ balance: string }>(
    `SELECT balance FROM wallets WHERE user_id = $1 AND asset = 'USDC'`,
    [userId]
  );
...
export async function initiateWithdrawal(
  ...
  try {
    ...
    await debit(userId, req.amountUsdc, withdrawalId, dbClient);

    // Insert Withdrawal record
    const result = await dbClient.query<WithdrawalRecord>(
    ...
    setImmediate(() => {
      import('../liquidity/liquidityManager').then(({ submitOrQueue }) => {
 ...
 * Requirements: 5.4
 */
export async function submitFlutterwavePayout(withdrawal: WithdrawalRecord): Promise<void> {
  ...
  const data = await res.json() as { status: string; message: string; data?: { id: string } };
  ...
  await pool.query(
    `UPDATE withdrawals SET flutterwave_ref = $1, updated_at = NOW() WHERE id = $2`,
    [flutterwaveRef, withdrawal.id]
  );
}
 ...
 */
export async function sendUserEmailNotification(userId: string, subject: string, message: string): Promise<void> {
  try {
 ...
 */
export async function retryWithdrawal(withdrawalId: string): Promise<void> {
  const MAX_RETRIES = 3;
 ...
 */
export async function refundWithdrawal(withdrawal: WithdrawalRecord): Promise<void> {
  const dbClient = await pool.connect();
  try {
    await dbClient.query('BEGIN');

    // Credit wallet back (also writes ledger entry via walletService.credit)
    await credit(withdrawal.userId, withdrawal.amountUsdc, withdrawal.id, dbClient);
  ...
  await sendUserEmailNotification(
    ...
    `Your withdrawal of ${withdrawal.amountUsdc} USDC has failed after multiple attempts. ` +
...
export async function handleFlutterwaveWebhook(
  ...
  const expectedHash = crypto
    .createHmac('sha256', config.flutterwaveWebhookSecret)
  ...
  const withdrawalResult = await pool.query<{
    ...
    `SELECT id, user_id, amount_usdc, status FROM withdrawals WHERE id = $1`,
    [withdrawalId]
  );

  if (withdrawalResult.rows.length === 0) return;

  const withdrawal = withdrawalResult.rows[0];
  if (withdrawal.status === 'completed') return; // already processed
  ...
  try {
    ...
    await dbClient.query(
      ...
      [withdrawal.user_id, withdrawal.amount_usdc, withdrawalId, balanceAfter]
    );
...
export async function getWithdrawal(
  ...
  const result = await pool.query<WithdrawalRecord & { owner_id: string }>(
    `SELECT
       id, user_id AS "userId", amount_usdc AS "amountUsdc", amount_ngn AS "amountNgn",
       fx_rate AS "fxRate", bank_code AS "bankCode", account_number AS "accountNumber",
       account_name AS "accountName", status, flutterwave_ref AS "flutterwaveRef",
  ...
  if ((withdrawal as unknown as { userId: string }).userId !== userId) {
