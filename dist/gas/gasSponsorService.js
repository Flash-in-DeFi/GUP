...
exports.buildAndSubmit = buildAndSubmit;
exports.getXlmBalance = getXlmBalance;
exports.checkXlmReserve = checkXlmReserve;
const stellar_sdk_1 = require("@stellar/stellar-sdk");
const env_1 = require("../config/env");
const STELLAR_NETWORK_PASSPHRASE = env_1.config.stellarNetwork === 'mainnet' ? stellar_sdk_1.Networks.PUBLIC : stellar_sdk_1.Networks.TESTNET;
/**
 ...
 */
async function buildAndSubmit(operations, signers) {
    const server = new stellar_sdk_1.Horizon.Server(env_1.config.stellarHorizonUrl);
    const sponsorKeypair = stellar_sdk_1.Keypair.fromSecret(env_1.config.gasSponsorSecret);
    // Load the sponsor account to get the current sequence number
    const sponsorAccount = await server.loadAccount(env_1.config.gasSponsorAddress);
    const txBuilder = new stellar_sdk_1.TransactionBuilder(sponsorAccount, {
 ...
 * Requirements: 6.4
 */
async function getXlmBalance() {
    const server = new stellar_sdk_1.Horizon.Server(env_1.config.stellarHorizonUrl);
    const account = await server.loadAccount(env_1.config.gasSponsorAddress);
 ...
 * Requirements: 6.4
 */
async function checkXlmReserve(server) {
    const horizonServer = server ?? new stellar_sdk_1.Horizon.Server(env_1.config.stellarHorizonUrl);
    const account = await horizonServer.loadAccount(env_1.config.gasSponsorAddress);
    const nativeBalance = account.balances.find((b) => b.asset_type === 'native');
    const balance = parseFloat(nativeBalance?.balance ?? '0');
    const minReserve = env_1.config.minXlmReserve;
    if (balance < minReserve) {
        const message = `[GasSponsor] ALERT: XLM reserve low! ` +
 ...
 * Requirements: 6.4
 */
async function sendOperatorAlert(currentBalance, minReserve) {
...
//# sourceMappingURL=gasSponsorService.js.map